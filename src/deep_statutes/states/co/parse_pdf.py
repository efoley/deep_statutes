...
def _to_header(text_range: tuple[int, int], root: Tree) -> Header:
    ...
    match tree.data:
        case "title_start":
            ...
            sub_text = _text(_child(tree, 1, "subtitle_upper"))
        ...
        case "part_start":
            text = _text(_child(tree, 0, "part_number"))
            sub_text = _text(_child(tree, 1, "subtitle_upper"))
            return Header(HeaderType.Part, text, sub_text, text_range)
        case "section_start":
        ...
        case _:
            raise ValueError(f"Unknown heading type: {tree.data}")
...
title_start: _SPAN_L title_number _LINE+ subtitle_upper
article_start: _SPAN_M_B article_number _LINE+ subtitle
part_start: _SPAN_M part_number _LINE+ subtitle_upper

_subtitle_upper_line: _INDENT* _m_upper_text
subtitle_upper: _sep{_subtitle_upper_line, _LINE}

_subtitle_line: _INDENT* _m_text
subtitle: _sep{_subtitle_line, _LINE}
